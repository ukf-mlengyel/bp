...
public class AddBinActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imageView = findViewById(R.id.add_bin_img_preview);
        editText = findViewById(R.id.add_bin_description);
        addButton = findViewById(R.id.add_bin_button);

        // switches
        for (int i = 0; i<8; i++){
            int check = (int)Math.pow(2, i);

            String name = "add_bin_filter_switch"+check;
            int id = getResources()
                    .getIdentifier(name, "id", getPackageName());

            filterSwitches[i] = findViewById(id);
        }
    ...
    public void addBin(View view) {
        ...
        for (int o = 0; o<8; o++){
            if (filterSwitches[o].isChecked()){
                types[0] += (int)Math.pow(2, o);
            }
        }

        if (types[0] == 0){
            Toast.makeText(getApplicationContext(), R.string.types_not_selected, Toast.LENGTH_LONG).show();
            return;
        }
        ...
        progress.setMessage(getString(R.string.progress_add_bin));
        progress.setCancelable(false);
        progress.show();

        MultipartRequestSession request = new MultipartRequestSession(Request.Method.POST, BM.SERVER_URL+"/api/addBin.php", getApplicationContext(),
                response -> {
                    String data = new String(response.data);

                    Log.i("BM_BIN_UPLOAD_RESPONSE", data);
                    if (data.substring(0,1).equals("1")){
                        Toast.makeText(getApplicationContext(), R.string.bin_added, Toast.LENGTH_LONG).show();
            ...
            protected Map<String, String> getParams(){
                ...
                params.put("types", String.valueOf(types[0]));
