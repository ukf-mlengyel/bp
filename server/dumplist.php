...
<div class="container-md">
    ...
    if($_SERVER["REQUEST_METHOD"] == "GET" && isset($_REQUEST["userid"]))
        echo getDumpList(intval($_REQUEST["userid"]));
    else if (isset($_SESSION["user_id"]))
        header('Location: dumplist.php?userid='.$_SESSION["user_id"]);
    else
        echo WebElements::errorBox(loginErrString);

    function getDumpList(int $userid) : string{
        ...
        $dumps = DumpDB::getDumpListForUser($userid, false);

        $dumpList = WebElements::profileList($dumps, "", "dump");
        ...
        return "
                ...
                <div class='container'>
                    <div class='row'>
                        ...
                        <div class='col'><h1>Skládky používateľa {$details["username"]}</h1></div>
                    </div>
                </div>
